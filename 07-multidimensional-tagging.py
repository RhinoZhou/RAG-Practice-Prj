...
def main():
    ...
    try:
        # 示例1：处理单个文件
        input_file = "中华人民共和国民法典_条款分割.json"
        if os.path.exists(input_file):
            ...
            # 导出结果
            output_file = "中华人民共和国民法典_标签标注.json"
            tagger.export_tagged_articles(output_file)
            
            # 导出元数据
            metadata_file = "中华人民共和国民法典_标签元数据.json"
        ...
        # 示例2：批量处理多个文件
        # 查找所有条款分割文件
        clause_files = [f for f in os.listdir('.') if f.endswith('_条款分割.json')]
