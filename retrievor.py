...
def search_web(query):
    """利用搜索引擎搜索接口，用于检索与query相关的背景信息，作为检索内容
    input：query
...
class TextRetrievalRanker():
    """
    实现对检索内容的召回与排序
    """

    def __init__(self, config):
        self.top_keyword_count = config.top_keyword_count    # 查询关键词召回数量
        self.top_doc_count = config.top_doc_count    # 召回文档数量
        self.top_text_count = config.top_text_count    # 召回文本片段数量
        self.max_text_length = config.max_text_length  # 召回文本片段长度
        self.retrieval_method = config.retrieval_method  # 召回方式



    def analyze_query(self, query):
        """查询分析，目前利用jieba进行关键词提取
        input: query, top_keyword_count
        output:
            keywords: 关键词列表，包含权重
            total_weight: 总权重
        """
        keywords = jieba.analyse.extract_tags(query, topK=self.top_keyword_count, withWeight=True)
        total_weight = self.top_keyword_count / sum([r[1] for r in keywords]) if keywords else 0
        return keywords, total_weight

    def split_text(self, text, max_length, separators='\n', strip_chars=None):
        """将文本按照标点符号划分为若干个短句
        """
        text = text.strip().strip(strip_chars)
        if separators and len(text) > max_length:
            pieces = text.split(separators[0])
            text, texts = '', []
            for i, p in enumerate(pieces):
                if text and p and len(text) + len(p) > max_length - 1:
                    texts.extend(self.split_text(text, max_length, separators[1:], strip_chars))
                    text = ''
                if i + 1 == len(pieces):
                    text = text + p
                else:
                    text = text + p + separators[0]
            if text:
                texts.extend(self.split_text(text, max_length, separators[1:], strip_chars))
            return texts
        else:
    ...
    def calculate_title_score(self, title, keywords, total_weight):
        """计算查询与标题的匹配度"""
        score = 0
    ...
    def calculate_text_score(self, text, keywords, total_weight):
        """计算查询与文本的匹配程度"""
        score = 0
        for item in keywords:
            kw, weight = item
    ...
    def rank_by_keywords(self, query, data):
        """通过关键词进行召回"""
        
        # 查询分析
        keywords, total_weight = self.analyze_query(query)
        
        # 先召回标题
        title_score = {}
        for line in data:
            title = line['title']
            title_score[title] = self.calculate_title_score(title, keywords, total_weight)
        title_score = sorted(title_score.items(), key=lambda x: x[1], reverse=True)
        # print(title_score)
        recall_title_list = [t[0] for t in title_score[:self.top_doc_count]]
        ...
        for line in data:
            ...
            if title in recall_title_list:
                for ct in self.split_text(text, self.max_text_length, separators='\n。'):
                    ct = re.sub('\s+', ' ', ct)
                    if len(ct) >= 20:
                        sentence_score[ct] = self.calculate_text_score(ct, keywords, total_weight)

        sentence_score = sorted(sentence_score.items(), key=lambda x: x[1], reverse=True)
        recall_sentence_list = [s[0] for s in sentence_score[:self.top_text_count]]
        return '\n'.join(recall_sentence_list)

    def rank_by_vector_similarity(self, query, data):
        """通过向量相似度进行召回"""
        if not data:
        ...
        title_vectors = get_text_embeddings(title_list, 8)
        ...
        title_score = get_similarity_scores(title_vectors)
        ...
        # 确保有足够的标题用于召回
        if not title_score or self.top_doc_count <= 0:
            print("Warning: No title scores or invalid top_doc_count parameter")
            return ""
            
        recall_title_list = [title_list[t[1]] for t in title_score[:min(self.top_doc_count, len(title_score))]]
        ...
        sentence_vectors = get_text_embeddings(sentence_list, 8)
        ...
        sentence_score = get_similarity_scores(sentence_vectors)
        ...
        # 确保有足够的句子用于召回
        if not sentence_score or self.top_text_count <= 0:
            print("Warning: No sentence scores or invalid top_text_count parameter")
            return ""
            
        recall_sentence_list = [sentence_list[s[1]] for s in sentence_score[:min(self.top_text_count, len(sentence_score))]]
        return '\n'.join(recall_sentence_list)
    ...
    def retrieve_relevant_text(self, query):
        ...
        if self.retrieval_method == 'keyword':
            bg_text = self.rank_by_keywords(query, data)
        else:
            bg_text = self.rank_by_vector_similarity(query, data)
        return bg_text


config = AppConfig()
trr = TextRetrievalRanker(config)
query_retriever = trr.retrieve_relevant_text
